...
def run(request):
    ...
    exc_occured = False
    try:
        file_bytes = request.get_data(False)    
        preds = model.predict(file_bytes)
    ...
    if exc_occured:
        try:
            file_bytes = request.get_json(force=True)
            image_data = file_bytes["image"]
            preds = model.predict(image_data)
            exc_occured = False
        except Exception as e:
            print("Exception occured in get_json(): ", e)
            exc_occured = True
